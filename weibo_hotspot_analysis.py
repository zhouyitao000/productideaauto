...
class WeiboHotspotAnalyzer:
    ...
    def _process_single_topic(self, topic: Dict[str, Any]) -> Dict[str, Any]:
        ...
        prompt = f"""
        角色: 你是一位经验丰富的独立开发者 (Indie Hacker) 和产品经理，擅长使用 Jobs-to-be-Done (JTBD) 框架发现市场机会。
        
        当前日期: {current_date_str}
        分析目标: {topic.get('source', '微博')}热搜话题 "{topic['title']}"
        
        搜索背景信息:
        {search_context}
        
        请按照以下思维链 (Chain of Thought) 进行深度分析，不要直接生成显而易见的创意：

        Step 1: 深度拆解 (The "Middle Layer")
        1. 情绪分析: 这个热点引发了什么核心情绪？(焦虑、愤怒、好奇、狂欢?) 谁是情绪最强烈的受众？
        2. 二阶效应: 这个事件在未来 3-6 个月会导致什么商业或生活方式的变化？(例如：政策变化导致合规成本增加 -> 需要咨询工具)
        3. 机会识别: 基于上述分析，寻找未被满足的微小需求。

        Step 2: 创意生成约束 (The "Filter")
        - 必须是 Micro-SaaS、浏览器插件、微信小程序、信息差工具或娱乐化内容站。
        - 严禁生成: 政府级监控平台、大型企业级系统、实体硬件产品、纯线下服务。
        - 区分来源:
          * 如果是社会/新闻热点: 侧重于解决焦虑、提供信息聚合、合规辅助工具。
          * 如果是娱乐/抖音热点: 侧重于娱乐模版、生成器、滤镜、趣味测试、粉丝互动工具。

        Step 3: 生成 2 个具体的产品创意
        创意必须落地，适合个人开发者在 2-4 周内完成 MVP。

        输出格式 (仅JSON):
        {{
            "research": {{
                "summary": "综合摘要 (包含情绪分析和二阶效应思考，150字左右)"
            }},
            "creatives": [
                {{
                    "name": "创意名称 (要在名字里体现产品形态，如'xx助手'、'xx插件')",
                    "user_pain_point": "用户核心痛点 (JTBD: 用户雇佣该产品是为了解决什么问题?)",
                    "features": ["核心功能1", "核心功能2"],
                    "monetization": "变现模式 (如: 付费订阅、一次性买断、广告、引流)",
                    "target_users": "具体细分人群 (如: 留学生家长、外贸SOHO、追星族)",
                    "scores": {{
                        "interest": 85,
                        "usefulness": 70
                    }},
            ...
            for result in batch_results:
                ...
                for i, creative in enumerate(creatives):
                    ...
                    creative_data = {
                        ...
                        "monetization": creative.get('monetization', '未知'),
                        "user_pain_point": creative.get('user_pain_point', '未知'),
                        "scores": scores,
                    ...
                    <div class="creative-card" data-id="{creative_id}">
                        ...
                        <div class="user-pain-point" style="margin-bottom: 0.5rem; color: #666; font-style: italic;">
                            <strong>💡 核心痛点 (JTBD):</strong> {creative.get('user_pain_point', '暂无描述')}
                        </div>
                        ...
                        <div class="target-users">
                            ...
                            <strong>💰 变现模式:</strong> {creative.get('monetization', '未知')}
                        </div>
                        {competitors_html}
